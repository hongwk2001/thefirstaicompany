...
SCRIPT_FILE = "d:/git_repo/thefirstaicompany/secret_garden_pilot_script.json"
OUTPUT_FILE = "d:/git_repo/thefirstaicompany/secret_garden_pilot.mp3"
TEMP_DIR = "d:/git_repo/thefirstaicompany/temp_audio"

def check_mary_files():
    required_files = [
        "mary_voice/mary_1.mp3",
        "mary_voice/mary_2.mp3"
    ]
    missing = []
    for rf in required_files:
        path = os.path.join("d:/git_repo/thefirstaicompany", rf)
...
def fetch_elevenlabs_audio(text, voice_id, index):
    ...
    print(f"Generating audio segment {index} ({voice_id})...")
    ...
    if response.status_code == 200:
        ...
        print(f"Error generating segment {index}: {response.status_code} - {response.text}")
        return None

def stitch_audio_binary(files, output_path):
    print("Stitching audio files together using binary concatenation...")
    try:
...
def main():
    ...
    if missing_mary:
        print("ERROR: Missing custom audio files for Mary's voice:")
        for m in missing_mary:
            print(f"  - {m}")
        print("\nPlease generate Mary's lines on ElevenLabs using your custom voice, download them, and place them in the 'mary_voice' directory.")
        sys.exit(1)
    ...
    try:
        for idx, item in enumerate(script):
            ...
            if item.get("type") == "custom_file":
                # Copy path to Mary's custom file
                src_path = os.path.join("d:/git_repo/thefirstaicompany", item["file_path"])
                dest_path = os.path.join(TEMP_DIR, f"segment_{segment_idx:03d}.mp3")
                # Read and write to temp folder
